...
async def run_test():
    ...
    try:
        ...
        # Click on the Sign In button to navigate to login page
        ...
        # -> Locate and click the logout button to initiate logout.
        frame = context.pages[-1]
        # Click on the user profile or menu to find logout option
        elem = frame.locator('xpath=html/body/div[2]/main/section[3]/div/div[2]/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click on the user profile button at index 4 to open the user menu and reveal the logout option.
        frame = context.pages[-1]
        # Click on the user profile button to open user menu for logout option
        elem = frame.locator('xpath=html/body/div[2]/header/div/div/div[2]/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click the Sign out button at index 4 to log out the user.
        frame = context.pages[-1]
        # Click the Sign out button to log out the user
        elem = frame.locator('xpath=html/body/div[2]/header/div/div/div[2]/div/div[2]/button[3]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Verify session invalidation by attempting to access a protected page or re-login with the same credentials.
