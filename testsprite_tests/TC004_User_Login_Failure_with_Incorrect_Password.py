...
async def run_test():
    ...
    try:
        ...
        # -> Check browser storage (cookies/local storage) to confirm no authentication token was issued and verify no redirect or successful login state.
        frame = context.pages[-1]
        # Click Sign In button again to see if error message appears or changes
        elem = frame.locator('xpath=html/body/div[2]/div/form/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
