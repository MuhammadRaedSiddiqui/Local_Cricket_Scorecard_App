...
async def run_test():
    ...
    try:
        ...
        # -> Click on 'Create a Scorecard' button to start a new match scoring session.
        frame = context.pages[-1]
        # Click on 'Create a Scorecard' button to start scoring.
        elem = frame.locator('xpath=html/body/main/section[6]/div[3]/div/div/div[2]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Fill the registration form with Full Name, Email, Password, Confirm Password and submit to create an account.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        # Input Email Address in registration form
        ...
        frame = context.pages[-1]
        # Input Confirm Password in registration form
        ...
        # -> Click on 'Sign in' link to navigate to the login page and proceed with login.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Latency Threshold Exceeded').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test plan failed: Scoring actions did not meet the required 150ms median latency under realistic loads.")
