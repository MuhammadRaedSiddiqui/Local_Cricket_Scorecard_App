...
export default function LeaderboardTabs({
  ...
  return (
    <div className="w-full">
      ...
      {/* Tabs */}
      <div className="bg-white rounded-2xl p-2 mb-8 border border-gray-200 shadow-xl">
        <div className="flex gap-2">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id as 'batting' | 'bowling' | 'teams')}
              className={`flex-1 relative px-6 py-4 rounded-xl font-bold transition-all duration-300 ${activeTab === tab.id
                  ? 'text-white'
